...
CREATE TABLE `nl_tablestructure_info` (
    ...
    `update_time` datetime DEFAULT NULL COMMENT '更新时间',
    `creator_id` varchar(50) DEFAULT NULL COMMENT '创建者ID',
    `is_create` tinyint(4) DEFAULT NULL COMMENT '是否创建表结构',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `nl_tablefield_info` (
    ...
    `field_decimal_length` int(11) DEFAULT NULL COMMENT '类型小数长度',
    `field_is_key` tinyint(4) DEFAULT NULL COMMENT '是否为主键',
    `allow_null` tinyint(4) NOT NULL COMMENT '是否允许为空',
    `field_default_value` varchar(50) DEFAULT NULL COMMENT '字段默认值',
    `creator_id` varchar(50) DEFAULT NULL COMMENT '创建者ID',
    PRIMARY KEY (`id`)
...
CREATE TABLE `nl_app_relation_tablespace_info` (
    ...
    `app_id` int(11) NOT NULL COMMENT '主键ID',
    `tablespace_id` int(11) NOT NULL COMMENT '主键ID',
